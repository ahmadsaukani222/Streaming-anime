...
export default function GlobalChat() {
  ...
  return createPortal(
    <>
      {/* Floating Chat Button - Using CSS transitions instead of Framer Motion */}
      <button
        onClick={handleToggle}
        className="fixed bottom-24 sm:bottom-20 right-4 z-50 group transition-transform duration-200 hover:scale-105 active:scale-95"
      ...
      >
        ...
        {/* Unread Badge - CSS animation */}
        {unreadCount > 0 && (
          <span className="absolute -top-1 -right-1 min-w-[20px] h-5 px-1.5 bg-red-500 rounded-full flex items-center justify-center text-[11px] text-white font-bold border-2 border-[#0F0F1A] animate-bounce-in">
      ...
      <div
        className={`fixed bottom-40 sm:bottom-36 right-4 z-50 w-[380px] max-w-[calc(100vw-2rem)] h-[520px] max-h-[calc(100vh-12rem)] sm:max-h-[calc(100vh-10rem)] bg-[#0F0F1A]/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/10 flex flex-col overflow-hidden transition-all duration-300 ease-out ${isOpen
