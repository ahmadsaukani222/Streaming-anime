            ...
            {isDebouncing ? (
                ...
                <>
                    {filteredResults.map((anime) => (
                        ...
                        >
                            <div 
                                className="w-12 h-16 rounded-lg flex-shrink-0 bg-white/5 overflow-hidden flex items-center justify-center"
                                style={{ minWidth: '48px', minHeight: '64px' }}
                            >
                                <img
                                    src={anime.poster}
                                    alt={anime.title}
                                    ...
                                    loading="lazy"
                                    onError={(e) => {
                                        console.warn('[Search] Failed to load poster:', anime.poster);
                                        e.currentTarget.style.display = 'none';
                                    }}
                                />
                            </div>
                            <div className="flex-1 min-w-0 py-1">
                                <p className="text-white font-medium line-clamp-1 text-base">{anime.title}</p>
                                <div className="flex items-center gap-2 mt-1.5">
                                    <span className="text-white/50 text-sm">{anime.releasedYear}</span>
                                    <span className={`text-xs px-2 py-0.5 rounded-full ${anime.status === 'Ongoing'
